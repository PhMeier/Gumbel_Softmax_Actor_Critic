...
def main():
    ...
    value_network_optim = torch.optim.Adam(value_network.parameters(),lr=1e-3) # 0.01

    # init actor network
    actor_network = ActorNetwork(STATE_DIM,40,ACTION_DIM)
    actor_network_optim = torch.optim.Adam(actor_network.parameters(),lr = 1e-3) # 0.01
    ...
    for i in range(2):
        while True:
            ...
            #action = torch.argmax(softmax_action).item()
            action = np.argmax(softmax_action.data.numpy()[0])
