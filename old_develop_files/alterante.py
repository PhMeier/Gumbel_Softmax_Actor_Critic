...
def roll_out(actor_network,task,sample_nums,value_network,init_state):
    ...
    for j in range(sample_nums):
        ...
        if done:
            state = task.reset()
            break
    return states,actions,rewards,final_r,state, done
...
def main():
    ...
    while True:
            states, actions, rewards, final_r, current_state, done = roll_out(actor_network,task,SAMPLE_NUMS,value_network,init_state)
