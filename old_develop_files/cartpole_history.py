...
def main():
    ...
    for x in range(epoch):
        ...
        x = sum(collected_vs)
        collected_vs = torch.stack(collected_vs)
        actor_network_loss = -sum(collected_vs) # calculate actor loss
        ...
        value_network_loss = criterion(sum(collected_values), sum(collected_rewards))  # 0.5 * torch.mean(torch.pow(values - rew, 2))
